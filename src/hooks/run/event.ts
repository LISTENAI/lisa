...
const event = async function () {
  if (!['login', 'create', '-v', '--version', '-h', '--help'].includes(this.config.id)) {
    ...
    if (accessToken) {
      try {
        this.debug('>>> 获取request [%s]：%s (%s)', this.config.id, JSON.stringify(params), accessToken)
